...
{
    std::string line, device_id, opcode, offset, length, timestamp;
    ...
    for (size_t pos = start; pos < end;)
    {
        ...
        if (std::getline(linestream, device_id, ',') &&
            std::getline(linestream, opcode, ',') &&
            std::getline(linestream, offset, ',') &&
            ...
            std::getline(linestream, timestamp, ','))
        {
            ...
            std::string key = device_id + ":" + offset;
