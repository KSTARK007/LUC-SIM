...
// 202401 process_chunck
void process_chunk(const char *data, size_t start, size_t end, std::vector<std::string> &output_lines)
{
    std::string line, op_time, key, key_size, op, op_count, size, cache_hits, ttl, usecase, sub_usecase;
    ...
    for (size_t pos = start; pos < end;)
    {
        ...
        if (std::getline(linestream, op_time, ',') &&
            std::getline(linestream, key, ',') &&
            std::getline(linestream, key_size, ',') &&
            std::getline(linestream, op, ',') &&
            std::getline(linestream, op_count, ',') &&
            std::getline(linestream, size, ',') &&
            std::getline(linestream, cache_hits, ',') &&
            std::getline(linestream, ttl, ',') &&
            std::getline(linestream, usecase, ',') &&
            std::getline(linestream, sub_usecase, ','))
        {
            // Convert operation to lowercase for uniform comparison
...
// 202206 process_chunck
// void process_chunk(const char *data, size_t start, size_t end, std::vector<std::string> &output_lines)
// {
//     std::string line, key, op, size, op_count, key_size;
...
//             // Process only "GET" operations
//             if (op == "get")
//             {
//                 freq_map[key] += std::stoi(op_count); // Increment count by `op_count`
//                 output_lines.push_back(key);
//             }
//         }
//         processed_lines++;
